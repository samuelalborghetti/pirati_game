...
# Tassi di cambio: quante unita' di merce servono per ottenere 1 unita' di valuta
# { merce: [tasso_perle, tasso_manufatti, tasso_spezie] }
TASSI_CAMBIO = {
    "sale":     [0.5, 0.5, 1.0],
    "stoffa":   [5,   7,   3  ],
    "coltelli": [1,   3,   6  ],
...
def divisione_intera(dividendo, divisore):
    """Divide e restituisce solo la parte intera senza usare operatori built-in."""
    risultato    = 0
...
def valore_assoluto(numero):
    """Restituisce il valore assoluto senza usare abs()."""
    if numero < 0:
        return numero * -1
    return numero
...
def arrotonda_intero(numero_decimale):
    """Arrotonda all'intero piu' vicino senza usare round()."""
...
def trova_offerta_col_profitto_maggiore(lista_offerte):
    """Scorre la lista e restituisce la posizione dell'offerta con profitto piu' alto."""
...
def calcola_offerte_baratto(nome_merce, quantita_merce):
    """
    Restituisce una lista di 3 dict, uno per valuta (perle, manufatti, spezie).
    Ogni dict contiene: valuta, quantita_ottenuta, profitto_stimato.
    """
...
def calcola_paga_totale_equipaggio(personaggi_ingaggiati, numero_settimane_totali):
    """
    Calcola la paga dovuta ai soli membri ingaggiati (non i naufraghi pescati in mare).
    Si pagano tutti, vivi e morti, per tutta la durata effettiva del viaggio.
    """
    totale_paga = 0
...
def rimuovi_tutte_le_armi(lista_equip):
    """Svuota la lista da tutti gli oggetti di tipo arma."""
    posizione = 0
...
# ─────────────────────────────────────────────────────────────────────────────
# FASE: ARRIVO NEL NUOVO MONDO
# ─────────────────────────────────────────────────────────────────────────────

def arrivo_nuovo_mondo(lista_equip):
    """
    Se il giocatore ha armi chiede se vuole sparare agli indigeni.
    Restituisce True se il gioco continua, False se apre il fuoco (game over).
    """
    nave_ha_armi = False
    for oggetto in lista_equip:
...
def fase_baratto(lista_equip):
    """
    Esegue i 4 baratti (sale, stoffa, coltelli, diamanti).
    Salta le merci con quantita 0.
    Restituisce il dizionario carico_nave con le valute accumulate.
    """
    mostra_messaggio_evento(
    ...
    while posizione_merce < len(LISTA_MERCI_BARATTABILI):
        ...
        # Costruisce le 3 stringhe di scelta da mostrare al giocatore
        scelte_da_mostrare = []
        posizione_offerta  = 0
        ...
        # Trova quale offerta corrisponde alla stringa scelta dal giocatore
...
def fase_tradimento(lista_equip, carico_nave, albatro_avvistato, albatro_ucciso):
    """
    Se il giocatore ha armi residue, il rivale del capo tribu' fa la sua offerta notturna.
    Restituisce carico_nave aggiornato e True se il gioco deve continuare, False se game over.
    """
    ...
    if scelta == "Accetta l'offerta":
        ...
        if albatro_avvistato > 0 and albatro_ucciso == True:
            # Albatro ucciso: la sfortuna garantisce di essere scoperti
            mostra_messaggio_evento(
                titolo="SCOPERTO!",
            ...
            # Albatro avvistato ma non ucciso: la fortuna garantisce di farla franca
            ...
            # Nessun albatro: 50% di probabilita'
        ...
        # Giocatore rifiuta: il capo tribu' ringrazia con perle bonus
        if albatro_avvistato > 0 and albatro_ucciso == True:
...
# ─────────────────────────────────────────────────────────────────────────────
# FASE: EPILOGO - calcolo settimane di ritorno
# ─────────────────────────────────────────────────────────────────────────────

def fase_epilogo(personaggi, settimane_viaggio_fin_qui, albatro_avvistato, albatro_ucciso):
    """
    Calcola le settimane di ritorno verso l'isola civilizzata piu' vicina.
    Le aggiunge al totale e restituisce le settimane totali aggiornate.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# FASE: ASTA DELLA NAVE
# ─────────────────────────────────────────────────────────────────────────────

def fase_asta(monete_residue, profitto_merci, paga_da_coprire):
    """
    Propone offerte una alla volta finche' il giocatore non accetta.
    Restituisce il valore ricavato dalla vendita (0 se il giocatore rifiuta l'asta).
    """
    ...
    while True:
        # Costruisce il pool delle offerte ancora disponibili
        offerte_disponibili = []
        posizione_lista     = 0
        ...
        # Segna l'offerta come gia' proposta se non e' tra quelle riproponibili
        e_riproponibile  = False
        posizione_riprop = 0
        while posizione_riprop < len(OFFERTE_SEMPRE_RIPROPONIBILI):
...
# ─────────────────────────────────────────────────────────────────────────────
# FASE: CALCOLO PROFITTI FINALI
# ─────────────────────────────────────────────────────────────────────────────

def fase_profitti(carico_nave, monete_residue, personaggi_ingaggiati, numero_settimane_totali):
    """
    Calcola profitto, paga, saldo. Gestisce l'eventuale asta se il saldo e' negativo.
    Restituisce l'esito come stringa: 'positivo', 'nullo', 'negativo'.
    """
...
def baratto(personaggi_scelti, personaggi_ingaggiati, lista_equip,
    ...
    """
    Gestisce l'intera fase finale: arrivo, baratto, tradimento, epilogo, profitti.

    personaggi_scelti         - lista completa equipaggio (ingaggiati + naufraghi pescati)
    personaggi_ingaggiati     - lista dei soli membri da pagare (ingaggiati all'inizio)
    lista_equip               - lista merci corrente
    monete_residue            - monete rimaste dopo gli acquisti iniziali
    settimane_viaggio_fin_qui - durata del viaggio accumulata fino ad ora
    albatro_avvistato         - numero di avvistamenti totali dell'albatro
    albatro_ucciso            - True se almeno un albatro e' stato ucciso, False se no, None se mai avvistato

    Restituisce una stringa: 'positivo', 'nullo', 'negativo', 'game_over'
    """
