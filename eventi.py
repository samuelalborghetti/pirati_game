...
# Tutte queste funzioni leggono i campi senza usare .get() e senza underscore nei for.
...
def leggi_morale(personaggio):
    if "morale" in personaggio:
        return personaggio["morale"]
    return personaggio["stats"]["morale"]

def scrivi_morale(personaggio, valore):
    if "morale" in personaggio:
        personaggio["morale"] = max(0, min(100, valore))
    else:
        personaggio["stats"]["morale"] = max(0, min(100, valore))
...
# ─── FUNZIONI RIUTILIZZABILI PER GLI EVENTI ───────────────────────────────────
...
def gestisci_razioni_interattivo(lista_cibo, personaggi, settimane_rimaste, razioni_attuali):
    num_membri   = conta_membri_vivi(personaggi)
    delta_morale = 0
...
def aggiorna_morale_equipaggio(personaggi, delta_morale):
    for p in personaggi:
        if e_vivo(p):
            scrivi_morale(p, leggi_morale(p) + delta_morale)
    return personaggi

def applica_morti_morale_zero(personaggi):
    morti = []
    for p in personaggi:
        if e_vivo(p) and leggi_morale(p) <= 0:
            uccidi(p)
            morti.append(leggi_nome(p))
    return morti
...
def step_ricalcolo_settimane(personaggi, settimane_totali):
    ...
    for p in personaggi:
        if e_vivo(p):
            numero_vivi += 1
            if leggi_morale(p) <= SOGLIA_MORALE_BASSO:
