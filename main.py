...
costo_settimanale_ingaggiati = [p["stats"]["cost"] for p in PERSONAGGI_SCELTI]
...
# --- MODO CORRETTO DI CARICARE LE MERCI ---
lista_merci = []
for nome in equip_scelto:
    for e in MERCI:
        if e["info"]["name"] == nome:
            ...
            nuova_merce = {
                "info": e["info"],                
                "stats": copy.deepcopy(e["stats"]),     
            }
            if "sprites" in e:
                nuova_merce["sprites"] = e["sprites"]
                
            lista_merci.append(nuova_merce)
...
def aggiorna_saturazione(verdura_totale, acqua_totale, carne_totale, frutta_totale):
    return verdura_totale + acqua_totale + carne_totale + frutta_totale
...
def draw_equip_info_box(screen, mouse_pos, medicinali_totali, armi_totali, strumenti_totali, rect_bt):
    if rect_bt.collidepoint(mouse_pos):
        ...
        strum_text = title_font.render(f"Totale merci: {strumenti_totali}", True, BIANCO)
        screen.blit(titolo,     (rect_info.x + 10*MOD, rect_info.y + 10*MOD))
...
while running:
    ...
    if schermata == 1:
        ...
        if not animazione_attiva:
            ...
            # FIX: rimosso il blocco cuoco con "role" (typo — non funzionava mai)
            # FIX: rimosso il blocco "morte per inedia" (20% random — non previsto dal PDF)
            ...
            for nome, base in consumi_base.items():
                ...
                if scorte_attuali[nome] <= 0:
                    ...
                    # FIX: rimosso il "-1" spurio che alterava il calcolo
                    futuro_necessario = base * moltiplicatore * vivi * settimane_rimaste

                    if scorte_attuali[nome] < futuro_necessario:
                        stato_attuale = "GIA' DIMEZZATA" if moltiplicatore < 1.0 else "NORMALE"
                        scelta = mostra_messaggio_evento(
                            f"SCARSEGGIA {nome.upper()}",
                            f"Residuo: {scorte_attuali[nome]:.1f} | Servono: {futuro_necessario:.1f}",
                            f"Razione attuale: {stato_attuale}. Dimezzare? (-5 morale/sett)",
                            ["Dimezza", "Mantieni"]
                        )
                        if scelta == "Dimezza":
                        ...
                        stato_attuale = "GIA' RADDOPPIATA" if moltiplicatore > 1.0 else "NORMALE"
                        scelta = mostra_messaggio_evento(
                            f"ABBONDANZA {nome.upper()}!",
                            f"Residuo: {scorte_attuali[nome]:.1f} | Doppio necessario: {futuro_necessario*2:.1f}",
                            f"Razione attuale: {stato_attuale}. Raddoppiare? (+5 morale/sett)",
                            ["Raddoppia", "Mantieni"]
                        )
                        if scelta == "Raddoppia":
            ...
            for p in PERSONAGGI_SCELTI:
                if p["stats"]["alive"]:
                    ...
                    p["stats"]["morale"] = max(0, min(100, morale_corrente + diminuzione_morale))
            ...
            # --- STEP 4: RIEPILOGO FINE SETTIMANA (Senza join) ---
            totale_medicinali, totale_armi, totale_merci = carica_totali_equip(lista_merci)
            ...
            # Creiamo una lista pulita di informazioni
            info_membri = []
            for p in PERSONAGGI_SCELTI:
                if p["stats"]["alive"]:
                    # Aggiungiamo ogni membro come elemento separato
                    info_membri.append(f"{p['info']['name']} (Morale: {p['stats']['morale']})")

            # Se non c'è nessuno, mettiamo un avviso
            if not info_membri:
                info_membri.append("Nessun superstite")

            # Ora passiamo direttamente la lista o i primi elementi alla funzione
            # Se la tua funzione mostra_messaggio_evento accetta solo stringhe, 
            # puoi passare solo i primi due o tre per non eccedere lo spazio:
            dettaglio_morale = info_membri[0] if len(info_membri) > 0 else ""
            if len(info_membri) > 1:
                dettaglio_morale += " | " + info_membri[1]

            mostra_messaggio_evento(
                f"FINE SETTIMANA {settimana_corrente}",
                f"Vivi: {vivi_count}  |  Cibo: {saturazione_totale:.1f}  |  Merci: {totale_merci}",
                f"Stato: {dettaglio_morale}"
            )
            ...
            if ammutinamento:
                ...
                running = False  # qui andrà la chiamata a nuovo_mondo.py
