...
def schermata_game_over(personaggi, razioni, merce, settimana, causa=""):
    ROSSO_SCURO = (120, 20, 20)
    ROSSO = (200, 60, 60)
    GRIGIO = (160, 140, 120)
    NERO_TRASP = (10, 5, 0)

    n_morti = sum(1 for p in personaggi if not p["stats"]["alive"])
    n_vivi = sum(1 for p in personaggi if p["stats"]["alive"])

    pw, ph = int(520*MOD), int(460*MOD)
    px = WIDTH // 2 - pw // 2
    py = HEIGHT // 2 - ph // 2

    btn_rigioca = pygame.Rect(px + int(40*MOD), py + ph - int(70*MOD), int(180*MOD), int(48*MOD))
    btn_esci    = pygame.Rect(px + pw - int(220*MOD), py + ph - int(70*MOD), int(180*MOD), int(48*MOD))
    ...
    while aspettando:
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
                ...
                if btn_rigioca.collidepoint(mouse):
                    return True   # rifare la partita
                if btn_esci.collidepoint(mouse):
                    return False  # uscire

        schermo.fill((8, 3, 3))

        s = pygame.Surface((pw, ph), pygame.SRCALPHA)
        pygame.draw.rect(s, (10, 5, 0, 220), (0, 0, pw, ph), border_radius=14)
        schermo.blit(s, (px, py))
        pygame.draw.rect(schermo, ROSSO_SCURO, (px, py, pw, ph), 3, border_radius=14)

        t_titolo = font_numeri.render("TUTTI MORTI", True, ROSSO)
        schermo.blit(t_titolo, t_titolo.get_rect(centerx=px + pw//2, y=py + int(20*MOD)))

        pygame.draw.line(schermo, ROSSO_SCURO,
                         (px + int(20*MOD), py + int(65*MOD)),
                         (px + pw - int(20*MOD), py + int(65*MOD)), 2)

        if causa:
            t_causa = title_font.render(causa, True, GRIGIO)
            schermo.blit(t_causa, t_causa.get_rect(centerx=px + pw//2, y=py + int(75*MOD)))

        t_sett = title_font.render(f"Sopravvissuti alla settimana {settimana}", True, GRIGIO)
        schermo.blit(t_sett, t_sett.get_rect(centerx=px + pw//2, y=py + int(105*MOD)))

        pygame.draw.line(schermo, ROSSO_SCURO,
                         (px + int(20*MOD), py + int(135*MOD)),
                         (px + pw - int(20*MOD), py + int(135*MOD)), 1)

        t_eq = title_font.render("Stato equipaggio:", True, BIANCO)
        schermo.blit(t_eq, (px + int(30*MOD), py + int(148*MOD)))

        y_off = py + int(175*MOD)
        for p in personaggi:
            nome = p["info"]["name"]
            vivo = p["stats"]["alive"]
            morale = p["stats"].get("morale", 0)
            col = GRIGIO if not vivo else BIANCO
            stato_str = "morto" if not vivo else f"vivo  morale: {morale:.0f}"
            simbolo = "X" if not vivo else "V"
            riga = title_font.render(f"  {simbolo}  {nome}: {stato_str}", True, col)
            schermo.blit(riga, (px + int(30*MOD), y_off))
            y_off += int(28*MOD)
            if y_off > py + ph - int(120*MOD):
                break

        pygame.draw.line(schermo, ROSSO_SCURO,
                         (px + int(20*MOD), py + ph - int(90*MOD)),
                         (px + pw - int(20*MOD), py + ph - int(90*MOD)), 1)

        t_ris = title_font.render(
            f"Risorse rimaste  cibo: {razioni['verdura']+razioni['frutta']+razioni['carne']:.1f}  acqua: {razioni['acqua']:.1f}  medicinali: {merce['medicinali']}",
            True, GRIGIO)
        schermo.blit(t_ris, t_ris.get_rect(centerx=px + pw//2, y=py + ph - int(88*MOD)))

        mouse = pygame.mouse.get_pos()
        for btn, testo in [(btn_rigioca, "Rigioca"), (btn_esci, "Esci")]:
            col_bg = (80, 30, 10) if btn.collidepoint(mouse) else (40, 15, 5)
            col_brd = (200, 80, 40) if btn.collidepoint(mouse) else ROSSO_SCURO
            pygame.draw.rect(schermo, col_bg, btn, border_radius=8)
            pygame.draw.rect(schermo, col_brd, btn, 2, border_radius=8)
            t_btn = title_font.render(testo, True, BIANCO)
            schermo.blit(t_btn, t_btn.get_rect(center=btn.center))
    ...
    if schermata == 1:
        ...
        if not animazione_attiva:
            ...
            if evento_estratto == "UOMO IN MARE":
                ...
                n_vivi = sum(1 for p in PERSONAGGI_SCELTI if p["stats"]["alive"])
                if n_vivi == 0:
                    vuole_rifare = schermata_game_over(
                        PERSONAGGI_SCELTI, razioni_attuali, merce_attuale,
                        settimana_corrente, causa="L'epidemia ha sterminato l'intero equipaggio.")
                    if vuole_rifare:
                        python = __import__("sys").executable
                        __import__("os").execv(python, [python] + __import__("sys").argv)
                    running = False
            ...
            if not running:
                pass  # game over gia' gestito sopra (epidemia con tutti morti)
            else:
                ...
                if n_vivi == 0:
                    vuole_rifare = schermata_game_over(
                        PERSONAGGI_SCELTI, razioni_attuali, merce_attuale,
                        settimana_corrente, causa="L'equipaggio e' perito in mare.")
                    if vuole_rifare:
                        python = __import__("sys").executable
                        __import__("os").execv(python, [python] + __import__("sys").argv)
                    running = False
            ...
            if not running:
                ...
                if running and settimana_corrente >= numero_settimane:
                    mostra_messaggio_evento(
